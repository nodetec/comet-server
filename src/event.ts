...
export function validateEventStructure(event: unknown): { valid: boolean; reason?: string } {
  ...
  if (typeof e.pubkey !== "string" || !/^[a-f0-9]{64}$/.test(e.pubkey)) {
    return { valid: false, reason: "invalid: pubkey is not a 64-char hex string" }
  }
  if (typeof e.sig !== "string" || !/^[a-f0-9]{128}$/.test(e.sig)) {
    return { valid: false, reason: "invalid: sig is not a 128-char hex string" }
  }
  ...
  if (typeof e.kind !== "number" || !Number.isInteger(e.kind) || e.kind < 0) {
    return { valid: false, reason: "invalid: kind is not a non-negative integer" }
  }
...
export function validateAndVerifyEvent(event: unknown): { ok: boolean; reason: string } {
