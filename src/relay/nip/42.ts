...
export function validateAuthEvent(
  ...
  if (Math.abs(now - e.created_at) > AUTH_WINDOW_SECONDS) {
    return { ok: false, reason: "invalid: AUTH event timestamp too far from current time" }
  }
...
export function isAuthorizedForFilter(
  ...
  if (authedPubkeys.size === 0) {
    return { authorized: false, reason: "auth-required: authentication required to query gift wraps" }
  }

  // Must have a #p filter
  const pValues = filter["#p"]
  if (!Array.isArray(pValues) || pValues.length === 0) {
    return { authorized: false, reason: "restricted: gift wrap queries must include a #p filter" }
  }

  // All p-tag values must be authed pubkeys
  for (const p of pValues) {
    if (!authedPubkeys.has(p as string)) {
      return { authorized: false, reason: "restricted: can only query gift wraps addressed to you" }
    }
  }
