import { eq, and, lt, lte, gte, desc, asc, inArray, exists, count, max, min, sql } from "drizzle-orm"
import type { NostrEvent, Filter, ChangeEntry, ChangesFilter } from "../types"
import type { DB } from "../db"
import { events, eventTags, deletedEvents, deletedCoords, changes, changeTags } from "../schema"
...
export interface Storage {
  ...
  processDeletionRequest(event: NostrEvent): Promise<{ deleted: number; changes: ChangeEntry[] }>
...
export function initStorage(db: DB): Storage {
  ...
  async function recordChange(
    ...
    const [row] = await tx.insert(changes).values({
      eventId,
      type,
      kind,
      pubkey,
  ...
  async function saveEvent(event: NostrEvent): Promise<{ saved: boolean; reason?: string; changes: ChangeEntry[] }> {
    ...
    if (isGiftWrap(event)) {
      ...
      if (pTag && dTag) {
        ...
        await db.transaction(async (tx) => {
          ...
          allChanges.push(await recordChange(tx as unknown as DB, event.id, "STORED", event.kind, event.pubkey, null, extractSingleLetterTags(event.tags)))
        })
    ...
    if (category === "replaceable") {
      ...
      await db.transaction(async (tx) => {
        const oldEvents = await tx
          ...
          .from(events)
          .where(and(eq(events.pubkey, event.pubkey), eq(events.kind, event.kind), lt(events.createdAt, event.created_at)))

        await tx.delete(events).where(and(eq(events.pubkey, event.pubkey), eq(events.kind, event.kind), lt(events.createdAt, event.created_at)))
        await insertEventWithTags(tx as unknown as DB, event)
        for (const old of oldEvents) {
          const oldTags = extractSingleLetterTags(old.tags as string[][])
          allChanges.push(await recordChange(tx as unknown as DB, old.id, "DELETED", old.kind, old.pubkey, { superseded_by: event.id }, oldTags))
        }
        allChanges.push(await recordChange(tx as unknown as DB, event.id, "STORED", event.kind, event.pubkey, null, extractSingleLetterTags(event.tags)))
      })
      return { saved: true, changes: allChanges }
    ...
    // Regular event
    const [existing] = await db.select({ id: events.id }).from(events).where(eq(events.id, event.id))
    if (existing) {
  ...
  async function queryEvents(filters: Filter[]): Promise<NostrEvent[]> {
    ...
    for (const filter of filters) {
      ...
      const limitClause = filter.limit != null ? `LIMIT ${Math.max(0, filter.limit)}` : ""
  ...
  async function processDeletionRequest(event: NostrEvent): Promise<{ deleted: number; changes: ChangeEntry[] }> {
    ...
    await db.transaction(async (tx) => {
      ...
      for (const targetId of targetIds) {
        const [targetEvent] = await tx
          .select({ pubkey: events.pubkey, kind: events.kind, tags: events.tags })
          .from(events)
        ...
        if (targetEvent) {
          if (targetEvent.kind === KIND_GIFT_WRAP) {
            authorized = canDeleteGiftWrap({ ...targetEvent, tags: targetEvent.tags as string[][] } as any, event.pubkey)
          } else {
        ...
        if (targetEvent && authorized) {
          const targetTags = extractSingleLetterTags(targetEvent.tags as string[][])
          await tx.delete(events).where(eq(events.id, targetId))
          deleted++
          allChanges.push(await recordChange(tx as unknown as DB, targetId, "DELETED", targetEvent.kind, event.pubkey, { deletion_id: event.id }, targetTags))
        }

        await tx.insert(deletedEvents).values({
          eventId: targetId,
          deletionId: event.id,
          pubkey: event.pubkey,
        }).onConflictDoNothing()
      ...
      for (const addr of addrTargets) {
        ...
        for (const a of affected) {
          ...
          allChanges.push(await recordChange(tx as unknown as DB, a.id, "DELETED", a.kind, a.pubkey, { deletion_id: event.id }, aTags))
        }
  ...
  async function queryChanges(filter: ChangesFilter): Promise<ChangeEntry[]> {
    ...
    return rows.map((row: any) => ({
      ...
      kind: row.kind,
      pubkey: row.pubkey,
      reason: row.reason ? (typeof row.reason === "string" ? JSON.parse(row.reason) : row.reason) : null,
    }))
  }
  ...
  return { saveEvent, queryEvents, deleteEvent, processDeletionRequest, isEventDeleted, queryChanges, getMaxSeq, getMinSeq, getEventCount }
}
