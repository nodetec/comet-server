...
import { parseNoteRumor, getRumorType, type Note } from "@/lib/rumor"
import type { NostrFilter } from "@/lib/nostr"
...
export function useNotes() {
  ...
  const query = useInfiniteQuery({
    ...
    queryFn: async ({ pageParam }): Promise<{ notes: Note[]; cursor: number | undefined }> => {
  ...
  const notes = useMemo(() => {
    ...
    return Array.from(noteMap.values()).sort((a, b) => b.modifiedAt - a.modifiedAt)
  }, [query.data])
