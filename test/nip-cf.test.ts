...
import { startTestRelay, connectWs, waitForMessage, waitForMessages, type TestContext } from "./helpers"
...
describe("isValidChangesFilter", () => {
  test("accepts valid filter", () => {
    ...
    expect(isValidChangesFilter({ since: 0, kinds: [1], authors: ["a".repeat(64)] })).toBe(true)
    expect(isValidChangesFilter({ since: 0, until_seq: 100, limit: 10 })).toBe(true)
...
describe("storage changelog", () => {
  ...
  test("queryChanges respects until_seq filter", async () => {
    const all = await ctx.storage.queryChanges({ since: 0 })
    const midSeq = all[Math.floor(all.length / 2)].seq
    const before = await ctx.storage.queryChanges({ since: 0, until_seq: midSeq })
